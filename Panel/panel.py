...
class AnimationPanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        # ANIMATION
        layout.prop(scene, "open_animation_manage", text="Manage Animation",
                    icon='TRIA_DOWN' if scene.open_animation_manage else 'TRIA_RIGHT')
        if scene.open_animation_manage:
            ...
            box.operator(
                "scene.join_anim", text="Join Animation").anim_name = scene.animation_settings.join_anim_name
            box.operator(
                "scene.rename_anim", text="Rename Animation").anim_name = scene.animation_settings.join_anim_name
            box.operator("scene.seperate_anim", text="Separate Animation")
            row.separator(factor=0.1)
            layout.separator(factor=0.1)

        layout.prop(scene, "open_animation_particle", text="Bake Particles",
                    icon='TRIA_DOWN' if scene.open_animation_particle else 'TRIA_RIGHT')
        ...
        layout.prop(scene, "open_animation_simplifly", text="Simplify Animation",
                    icon='TRIA_DOWN' if scene.open_animation_simplifly else 'TRIA_RIGHT')
        if scene.open_animation_simplifly:
            ...
            box = row.box()
            row = box.row()
            row.prop(scene.animation_settings,
                     "simplify_keyframes_enum", text="")
        ...
        layout.prop(scene, "open_uv_animation_menu", text="UV Animation",
                    icon='TRIA_DOWN' if scene.open_uv_animation_menu else 'TRIA_RIGHT')
...
class GLBExportPanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        layout.prop(scene, "open_verification_menu", text="Verification",
                    icon='TRIA_DOWN' if scene.open_verification_menu else 'TRIA_RIGHT')
        ...
        layout.prop(scene.export_settings, "open_export_settings_menu", text="Export Settings",
                    icon='TRIA_DOWN' if scene.export_settings.open_export_settings_menu else 'TRIA_RIGHT')
        if scene.export_settings.open_export_settings_menu:
            box = layout.box()
            # Scene Settings
            if scene.export_settings.open_scene_settings_menu:
                box.alert = True
            box.prop(scene.export_settings, "open_scene_settings_menu", text="Scene",
                     icon='TRIA_DOWN' if scene.export_settings.open_scene_settings_menu else 'TRIA_RIGHT')
            if scene.export_settings.open_scene_settings_menu:
                box.alert = False
                ...
                row.prop(scene.export_settings, "export_selected",
                         text="Selected Only", toggle=True, icon="RESTRICT_SELECT_OFF")
                row.prop(scene.export_settings, "export_lights",
                         text="Include Lights", toggle=True, icon="LIGHT")
                row.separator(factor=1)

                row = col.row(align=True)
                row.separator(factor=1)
                row.prop(scene.export_settings, "export_animations",
                         text="Include Animation", toggle=True, icon="RENDER_ANIMATION")
                row.prop(scene.export_settings, "apply_modifiers",
                         text="Apply Modifiers", toggle=True, icon="MODIFIER")
                row.separator(factor=1)

            # Animation Settings
            if scene.export_settings.open_animation_settings_menu:
                box.alert = True
            box.prop(scene.export_settings, "open_animation_settings_menu", text="Animation",
                     icon='TRIA_DOWN' if scene.export_settings.open_animation_settings_menu else 'TRIA_RIGHT')
            if scene.export_settings.open_animation_settings_menu:
                box.alert = False
                col = box.column(align=True)
                row = col.row(align=True)
                row.separator(factor=1)
                row.prop(scene.export_settings, "use_sampling",
                         text="Use Sampling", toggle=True, icon="OUTLINER_OB_CAMERA")
                row.prop(scene.export_settings, "group_by_nla",
                         text="Group by NLA", toggle=True, icon="NLA")
                row.separator(factor=1)
                row = col.row(align=True)
                row.separator(factor=1)
                row.prop(scene.export_settings, "export_all_influences",
                         text="Include all Bone Influences", toggle=True, icon="BONE_DATA")
                row.prop(scene.export_settings, "optimize_animation",
                         text="Optimize Animation", toggle=True, icon="KEYFRAME_HLT")
                row.separator(factor=1)

             # Compression Settings
            if scene.export_settings.open_compression_settings_menu:
                box.alert = True
            box.prop(scene.export_settings, "open_compression_settings_menu", text="Compression",
                     icon='TRIA_DOWN' if scene.export_settings.open_compression_settings_menu else 'TRIA_RIGHT')
            if scene.export_settings.open_compression_settings_menu:
                box.alert = False
                ...
                row = col.row(align=True)
                row.separator(factor=1)
                row.prop(scene.export_settings,
                         "export_image_format", text="Format")
                row.separator(factor=1)
                row = col.row()
                row.separator(factor=1)
                row.prop(scene.export_settings, "use_draco",
                         text="Use Draco", toggle=True)
                row.separator(factor=1)
                ...
                if scene.export_settings.use_draco:
                    ...
                    row.separator(factor=1)
                    row.prop(scene.export_settings,
                             "draco_compression_level", text="Compression Level")
                    row.prop(scene.export_settings, "postion_quantization",
                             text="Position Quantisation")
                    row.separator(factor=1)

                    row = col.row(align=True)
                    row.separator(factor=1)
                    row.prop(scene.export_settings,
                             "normal_quantization", text="Normal Quantisation")
                    row.prop(scene.export_settings, "texcoord_quantization",
                             text="Texture Coord. Quantisation")
                    row.separator(factor=1)

         # Optimization Settings
            if scene.export_settings.open_optimization_settings_menu:
                box.alert = True
            box.prop(scene.export_settings, "open_optimization_settings_menu", text="Optimization",
                     icon='TRIA_DOWN' if scene.export_settings.open_optimization_settings_menu else 'TRIA_RIGHT')
            if scene.export_settings.open_optimization_settings_menu:
                box.alert = False
                col = box.column(align=True)
                row = col.row(align=True)
                row.separator(factor=1)
                row.prop(scene.export_settings, "join_objects",
                         text="Join static objects", toggle=True, icon="SNAP_VERTEX")
                row.separator(factor=1)
...
class HelpPanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if lang == 'en_US':
            ...
            layout.operator("scene.open_link", text="Govie Platform",
                            icon='WORLD').url = "https://govie-editor.de/en"
